...
export function getArrow(change: number): { symbol: string; color: string } {
    if (change > 0) {
        return { symbol: "+", color: "var(--money-positive)" };
    } else if (change < 0) {
        return { symbol: "-", color: "var(--money-negative)" };
    }
    return { symbol: " ", color: "var(--money-neutral)" };
}
...
export function formatChangeText(change: number, percent: number): string {
    const arrow = getArrow(change);
    return `${arrow.symbol}${formatPrice(Math.abs(change))} (${formatPercent(
        Math.abs(percent)
    )})`;
}
...
export function formatChangeDisplay(
    ...
    const arrow = getArrow(change);
    return {
        text: `${arrow.symbol}${formatPrice(Math.abs(change))} (${formatPercent(
            Math.abs(percent)
        )})`,
        color: arrow.color,
    };
