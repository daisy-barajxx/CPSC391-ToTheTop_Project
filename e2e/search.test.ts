...
test.describe("Stock Search Page", () => {
    ...
    test("displays results when user searches for a valid stock", async ({
        page,
    }) => {
    ...
    test('displays "No results found" when search returns no matches', async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
    ...
    test("finds results regardless of case (uppercase, lowercase, mixed)", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
        ...
        const microsoftResult = page.getByText(
    ...
    test("trims whitespace from beginning and end of search term", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
    ...
    test("debounces requests - does not search until user stops typing", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
        ...
        // Type quickly: MS (2 keystrokes)
        // Without debouncing, this would trigger 2 API calls
        // With debouncing, it should only trigger 1
        await searchInput.pressSequentially("MS", { delay: 50 });

        // Wait for the results to load by looking for #search-results
        await page.waitForSelector("#search-results", { timeout: 1000 });

        // Expect results to be no more than 5 and include MSFT
    ...
    test("search results are clickable links to stock detail pages", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
        ...
        const teslaLink = page.getByRole("link", {
            name: /Tesla Inc\. \(TSLA\)/,
        });
    ...
    test("displays multiple results for partial name matches", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
        ...
        const resultsList = page.locator("ul#search-results li");
        const count = await resultsList.count();
        expect(count).toBeGreaterThan(1);
    });

    // TEST 9: Resetting search after previous search
    test("correctly resets and performs new search after clearing previous search", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
    ...
    test("shows loading indicator while fetching and hides when done", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
    ...
    test("triggers new search when user modifies search term", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
    ...
    test("handles special characters safely in search queries", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
        ...
        const noResultsOrResults = page.locator(
            "ul#results-list, #search-no-results"
        );
        await expect(noResultsOrResults).toHaveCount(1);
    });

    // TEST 13: Accessibility - aria labels
    test("has proper accessibility labels for screen readers", async ({
        page,
    }) => {
        const searchInput = page.getByPlaceholder(
            "Search stocks by name or symbol..."
    ...
    test("handles very long search terms gracefully", async ({ page }) => {
        ...
        const noResultsOrResults = page.locator(
            "ul#search-results, p#search-no-results"
        );
        await expect(noResultsOrResults).toHaveCount(1);
    });
});
